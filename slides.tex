...
\begin{column}{0.48\textwidth}
\begin{block}{\textbf{How MTZ prevents subtours}}
\small
If $x_{ij}=1$: constraint forces $u_j \geq u_i + 1$.\\[4pt]
Positions strictly increase along any used arc sequence.\\
A subtour would require positions to loop back --- impossible.
\end{block}

\vspace{4pt}

\begin{block}{\textbf{Properties}}
\small
\begin{itemize}\setlength\itemsep{2pt}
  \item $O(n^2)$ constraints --- polynomial
  \item Single solve, no iterations
  \item \textcolor{darkred}{Weak LP relaxation:} fractional $u_i$ values allow subtour-like fractional solutions $\Rightarrow$ more B\&B nodes
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\begin{enumerate}\setlength\itemsep{1pt}
  \item Solve ILP with C1--C6 only.
  \item BFS from $s$; find unreachable visited nodes $U$.
  \item If $U=\emptyset$: valid --- stop.
  \item Add one SEC per component of $U$.
  \item Re-solve. Repeat.
\end{enumerate}
\end{block}
\end{column}
...
\small
From $s$, at each step:\\[2pt]
1. Find unvisited neighbours within $R$ (not $t$).\\
2. If any lie in uncovered region $\to$ go to nearest such node.\\
3. Otherwise $\to$ go to nearest unvisited neighbour.\\
4. If no neighbour: Dijkstra escape on range graph.\\
5. Stop when all regions covered and $|\text{visited}|\geq V_{\min}$.\\
...
\small
Reverse interior segment $[v_i,\ldots,v_j]$ if:\\
total distance decreases \textbf{and} all new pairs satisfy $d\leq R$.\\
...
\small
For each interior node $v_\ell$, remove if:\\[2pt]
$\bullet$ $d(v_{\ell-1}, v_{\ell+1})\leq R$\\
$\bullet$ $|\text{visited}|-1 \geq V_{\min}$\\
$\bullet$ all regions still covered\\[3pt]
Remove node with largest distance saving. Repeat.
\end{block}

...
This problem is related to the Travelling Salesman Problem (TSP) studied in the CORO
...
technique~\cite{mtz}, which was presented in the CORO lectures in the context of
...
As discussed in the CORO lectures on the TSP, the MTZ formulation uses only
...
Johnson~\cite{dantzig}. This was also covered in the CORO lectures as the
...
upfront. This is the same situation encountered in the CORO lectures on cutting
...
version of the cutting plane algorithm from the CORO lectures on Gomory cuts and
SEC for the TSP:
...
In the CORO lectures, we also discussed a more sophisticated approach:
...
the notation from the CORO lectures on LP duality and relaxation quality:
\[
...
three phases: greedy construction, 2-opt local search, and node removal.
...
\begin{enumerate}[nosep]
  \item Compute the set of unvisited neighbours of the current node within range $R$.
  \item Among these, identify nodes in uncovered regions. If any exist, go to the
        nearest such node (this prioritises satisfying the region-covering
        constraint~\eqref{eq:cover}).
  \item If all reachable neighbours are in already-covered regions (or have no
        region), go to the nearest unvisited neighbour.
  \item Continue until all $m$ regions are covered \textbf{and} the path has at least
        $V_{\min}$ nodes.
  \item Connect to $t$: if $t$ is within range, go directly. Otherwise, use
        Dijkstra's algorithm on the range graph $G$ to find the shortest path to $t$.
\end{enumerate}

The greedy nearest-neighbour strategy is analogous to the classical TSP heuristic
discussed in the CORO lectures, extended with a priority mechanism for region
coverage. Its time complexity is $O(n^2)$.

\subsection{Phase~2: 2-opt Local Search}

Given the path $P = [v_1, v_2, \ldots, v_k]$, the 2-opt procedure considers all
pairs of interior indices $1 < i < j < k$ and tests reversing the segment
$[v_i, \ldots, v_j]$. The reversal is accepted if two conditions hold:
\begin{itemize}[nosep]
  \item Every new consecutive pair satisfies the range constraint:
        $\dist{v_{i-1}}{v_j} \leq R$, $\dist{v_i}{v_{j+1}} \leq R$, and all pairs
        within the reversed segment remain within $R$.
  \item The total distance strictly decreases:
        $\dist{v_{i-1}}{v_j} + \dist{v_i}{v_{j+1}}
        < \dist{v_{i-1}}{v_i} + \dist{v_j}{v_{j+1}}$.
\end{itemize}
The procedure repeats until no improving move is found. Each pass is $O(k^2)$.

\subsection{Phase~3: Node Removal}

After 2-opt, we attempt to shorten the path by removing unnecessary interior nodes.
For each $v_\ell$ with $1 < \ell < k$, we check:
\begin{itemize}[nosep]
  \item $\dist{v_{\ell-1}}{v_{\ell+1}} \leq R$ (the skip is within range),
  \item removing $v_\ell$ does not drop the visit count below $V_{\min}$,
  \item removing $v_\ell$ does not leave any region uncovered (i.e., another visited
        node covers the same region).
\end{itemize}
Among all removable nodes, we remove the one that saves the most distance:
$\dist{v_{\ell-1}}{v_\ell} + \dist{v_\ell}{v_{\ell+1}} -
\dist{v_{\ell-1}}{v_{\ell+1}}$. This is repeated until no beneficial removal exists.
...
terminology from the CORO lecture on branch-and-bound, the tree was not fully
...
The theoretical prediction from the CORO lectures is that SEC provides a tighter LP
...
capacity constraints from the CORO lecture on strengthening ILP formulations ---
...
\subsection{Connections to CORO Course Material}

The three approaches in this project connect directly to the CORO lecture topics:
\begin{itemize}[nosep]
